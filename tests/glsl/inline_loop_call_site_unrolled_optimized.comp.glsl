...
// This shader is the GLSL restatement of inline_loop_call_site_unrolled.slang.
// The unroller unrolls the loop in full. The one call site of Foo in the source
// therefore becomes two separate call sites in the compiled code, one for each
// unrolled iteration. Both call sites get inlined.
//
// Core GLSL cannot request unrolling, so this shader uses [[unroll]] from
// GL_EXT_control_flow_attributes. glslang unrolls nothing itself. The attribute
// makes glslang set the Unroll loop control bit on OpLoopMerge.
//
// The loop unroller of SPIRV-Tools does the real unrolling and inlining. The
// %spirv_opt step reaches that unroller. A loop without that bit is never
// unrolled, whatever flags spirv-opt is given.
...
// A DebugInlinedAt instruction describes the call site. This test captures its
// Line operand, which a debugger reports as the "inlined from" position.
//
...
//
// This test pins each access chain to its own constant index. A copy that reads
// or writes the wrong element therefore fails.
//
...
// enter it.
//
// glslang reports the body statement of Foo, and not its declaration line. Both
...
// CHECK: [[IN0:%[0-9]+]] = OpAccessChain {{.*}} %int_0 %uint_0
...
// CHECK: {{%[0-9]+}} = OpExtInst {{.*}} DebugLine {{%[a-zA-Z_0-9]+}} [[CALL_LINE]] [[CALL_LINE]]
// CHECK: [[OUT0:%[0-9]+]] = OpAccessChain {{.*}} %int_0 %uint_0
...
//
// The unroller can reuse the DebugInlinedAt instruction of the first iteration,
// or it can emit a new one. This test does not pin that choice. It pins the
// identity of Foo only. The line table must make the same excursion into Foo
// and back.
// CHECK: {{%[0-9]+}} = OpExtInst {{.*}} DebugValue [[I]] %uint_1
// CHECK: {{%[0-9]+}} = OpExtInst {{.*}} DebugLine {{%[a-zA-Z_0-9]+}} [[CALL_LINE]] [[CALL_LINE]]
// CHECK: [[IN1:%[0-9]+]] = OpAccessChain {{.*}} %int_0 %uint_1
...
// CHECK: {{%[0-9]+}} = OpExtInst {{.*}} DebugLine {{%[a-zA-Z_0-9]+}} [[CALL_LINE]] [[CALL_LINE]]
// CHECK: [[OUT1:%[0-9]+]] = OpAccessChain {{.*}} %int_0 %uint_1
