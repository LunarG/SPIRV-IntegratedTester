...
// inline_loop_call_site_unrolled_optimized.comp.glsl. That shader asks for the
// loop to be unrolled. This shader asks for the loop to be kept, with
// [[dont_unroll]] from GL_EXT_control_flow_attributes. A compiler must honor
// both requests, and the debug information must describe the shape that
// results.
//
// The explicit request matters. A loop with no attribute also survives this
// pass, but only because SPIRV-Tools does not unroll a loop that no shader
// asked it to unroll. That behavior is optimizer policy, and not a promise.
// [[dont_unroll]] makes the survival of the loop a contract instead.
...
// `i` is no longer in memory. This pass moved it to a loop-carried OpPhi. The
// incoming value of that phi from outside the loop is the same initial
// constant.
//
...
// CHECK: [[IN:%[0-9]+]] = OpAccessChain {{.*}} [[PHI]]
...
// iteration.
// CHECK: {{%[0-9]+}} = OpExtInst {{.*}} DebugLine {{%[a-zA-Z_0-9]+}} [[CALL_LINE]] [[CALL_LINE]]
// CHECK: [[OUT:%[0-9]+]] = OpAccessChain {{.*}} [[PHI]]
